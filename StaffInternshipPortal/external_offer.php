...
if (!empty($search_term)) {
    ...
    $extra_conditions .= " AND CONCAT(s.first_name, ' ', s.last_name) LIKE '%$search%'";
}

if (!empty($department_filter)) {
    $dep = mysqli_real_escape_string($conn, $department_filter);
    $extra_conditions .= " AND s.department_id = '$dep'";
}

if (!empty($major_filter)) {
    $maj = mysqli_real_escape_string($conn, $major_filter);
    $extra_conditions .= " AND s.major_id = '$maj'";
}

if (!empty($year_filter)) {
    $year = mysqli_real_escape_string($conn, $year_filter);
    $extra_conditions .= " AND s.academic_year = '$year'";
}

$base_query = "SELECT eo.*, CONCAT(s.first_name, ' ', s.last_name) AS student_name, s.profile_image, s.department_id 
               FROM external_offers eo
               INNER JOIN students s ON eo.student_id = s.student_id";

$pending_result = mysqli_query($conn, $base_query . " WHERE eo.status = 'Pending' " . $extra_conditions . " ORDER BY eo.created_at DESC");
$accepted_result = mysqli_query($conn, $base_query . " WHERE eo.status = 'Accepted' " . $extra_conditions . " ORDER BY eo.created_at DESC");
$rejected_result = mysqli_query($conn, $base_query . " WHERE eo.status = 'Rejected' " . $extra_conditions . " ORDER BY eo.created_at DESC");

$query_params = $_GET;
unset($query_params['tab']);
$filter_link = http_build_query($query_params);
?>
          ...
          <div class="right-side col-md-10 p-4">
              <div class="d-flex flex-column">
                ...
                <ul class="nav nav-pills mb-3 mt-4 border-bottom-color-876363" id="pills-tab" role="tablist">
                    <li class="nav-item">
                        <a href="?tab=pills-pending&<?= $filter_link ?>" class="nav-link <?= ($active_tab == 'pills-pending') ? 'active' : '' ?>">Pending</a>
                    </li>
                    <li class="nav-item">
                        <a href="?tab=pills-home&<?= $filter_link ?>" class="nav-link <?= ($active_tab == 'pills-home') ? 'active' : '' ?>">Accepted</a>
                    </li>
                    <li class="nav-item">
                        <a href="?tab=pills-profile&<?= $filter_link ?>" class="nav-link <?= ($active_tab == 'pills-profile') ? 'active' : '' ?>">Rejected</a>
                    </li>
                </ul>
